...
namespace CUVR{
    public class Interactable_Grabbable_spininplace : Interactable_rgbCube
    {
        bool grab;
        bool prevGrab;
        Transform oldParent;
            ...
            if (gaze != null)
            {
                if (!grab && prevGrab)
                {
                    if (this.GetComponent<Rigidbody>() != null)
                    {
                        ...
                        this.GetComponent<Rigidbody>().AddForceAtPosition((this.transform.position-prevPos)/Time.deltaTime, gaze.transform.position, ForceMode.VelocityChange);
                    }
